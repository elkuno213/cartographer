...
PaintSubmapSlicesResult PaintSubmapSlices(
  ...
  // Make bounding box and extend it based on size of submap slice.
...
void DeserializeAndFillSubmapSlices(
  ...
  while (deserializer->ReadNextSerializedData(&proto)) {
    if (proto.has_submap() && (Has2DGrid(proto.submap()) || Has3DGrids(proto.submap()))) {
      ...
      FillSubmapSlice(
        submap_poses.at(id), submap, &(*submap_slices)[id], conversion_tables
      );
    }
  }
}
...
UniqueCairoSurfacePtr DrawTexture(
  ...
  // Add data (intensity, alpha, observed) into cairo data.

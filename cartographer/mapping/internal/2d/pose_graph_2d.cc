...
std::vector<SubmapId> PoseGraph2D::InitializeGlobalSubmapPoses(
  ...
  CHECK(
    data_.submap_data.at(front_submap_id).submap == insertion_submaps.front()
  );
...
void PoseGraph2D::AddTrajectoryIfNeeded(const int trajectory_id) {
  ...
  CHECK(
    data_.trajectories_state.at(trajectory_id).state != TrajectoryState::DELETED
  );
  CHECK(
...
common::Time PoseGraph2D::GetLatestNodeTime(
  ...
  if (!submap_data.node_ids.empty()) {
    ...
    time = std::max(
      time, data_.trajectory_nodes.at(last_submap_node_id).constant_data->time
    );
  }
  return time;
}
  ...
  auto report_progress = [this,
    ...
    if (num_trajectory_nodes != num_finished_nodes_at_start) {
      ...
      progress_info
        ...
        << 100.
             * (constraint_builder_.GetNumFinishedNodes() - num_finished_nodes_at_start)
...
void PoseGraph2D::AddSerializedConstraints(
  ...
  AddWorkItem([this, constraints]() LOCKS_EXCLUDED(mutex_) {
    ...
    for (const auto& constraint : constraints) {
      ...
      CHECK(
        data_.trajectory_nodes.at(constraint.node_id).constant_data != nullptr
      );
...
std::vector<PoseGraphInterface::Constraint> PoseGraph2D::constraints() const {
  ...
  for (const Constraint& constraint : data_.constraints) {
    result.push_back(Constraint{
      ...
      Constraint::Pose{
        constraint.pose.zbar_ij
          * transform::Rigid3d::Rotation(data_.trajectory_nodes
...
MapById<SubmapId, PoseGraphInterface::SubmapData> PoseGraph2D::GetAllSubmapData(
) const {
  absl::MutexLock locker(&mutex_);
...
void PoseGraph2D::RegisterMetrics(metrics::FamilyFactory* family_factory) {
  ...
  auto* queue_size      = family_factory->NewGaugeFamily(
    "mapping_2d_pose_graph_work_queue_size", "Number of items in the work queue"
  );

...
RealTimeCorrelativeScanMatcher2D::GenerateExhaustiveSearchCandidates(
  ...
  // Calculate number of candidates. (each scan has a list of candidates based
  // on its linear bounds).
...
double RealTimeCorrelativeScanMatcher2D::Match(
  ...
  // Rotate initial point cloud by initial rotation.
  ...
  // Generate and discretize potential scans (translation inside).
  ...
  // Generate pose-related candidates.
  std::vector<Candidate2D> candidates
    = GenerateExhaustiveSearchCandidates(search_parameters);
  ScoreCandidates(grid, discrete_scans, search_parameters, &candidates);
  // Pick the best candidate based on its score as highest one.
  const Candidate2D& best_candidate
    = *std::max_element(candidates.begin(), candidates.end());
  // Move initial pose estimate to best candidate.

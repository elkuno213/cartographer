...
PrecomputationGridStack2D::PrecomputationGridStack2D(
  const Grid2D& grid, const proto::FastCorrelativeScanMatcherOptions2D& options
) {
...
bool FastCorrelativeScanMatcher2D::MatchWithSearchParameters(
  ...
  // Generate discretized scans by search params.
  ...
  // Compute lowest-resolution cadidates and find amongst them the best one by
  ...
  // Take only the best candidate whose score exeeds a min threshold.
...
FastCorrelativeScanMatcher2D::GenerateLowestResolutionCandidates(
  ...
  for (int scan_index = 0; scan_index != search_parameters.num_scans; ++scan_index) {
    for (
      int x_index_offset = search_parameters.linear_bounds[scan_index].min_x;
      x_index_offset <= search_parameters.linear_bounds[scan_index].max_x;
      x_index_offset += linear_step_size
    ) {
      for (
        int y_index_offset = search_parameters.linear_bounds[scan_index].min_y;
...
void FastCorrelativeScanMatcher2D::ScoreCandidates(
  ...
  // For each candidate, compute its score based on precomputation grid.
  ...
  // Sort candidate in non-ascending order by their scores.
  std::sort(
...
Candidate2D FastCorrelativeScanMatcher2D::BranchAndBound(
  ...
  // Return the best candidate if no depth.
  ...
  for (const Candidate2D& candidate : candidates) {
    // Stop if any candicate's score belows min threshold. (Candidates are
    // already sorted, so no need to continue the rest).
    if (candidate.score <= min_score) {
      break;
    }
    // Split candidate and find its inner candidates.
    std::vector<Candidate2D> higher_resolution_candidates;
    const int half_width = 1 << (candidate_depth - 1);
    for (int x_offset : {0, half_width}) {
      // Stop if out of x bound of search window.
      if (candidate.x_index_offset + x_offset > search_parameters.linear_bounds[candidate.scan_index].max_x) {
        break;
      }
      for (int y_offset : {0, half_width}) {
        // Stop if out of y bound of search window.
    ...
    // Score these inner candidates (sort inside).
    ScoreCandidates(
    ...
    // Recursion: find the best candidate between the current one and all inner
    // ones (min_score is the score of the currently best candidate).

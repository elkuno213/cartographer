...
void ConstraintBuilder2D::MaybeAddConstraint(
  ...
  // No constraint added if relative translation exceeds threshold.
  if (initial_relative_pose.translation().norm() > options_.max_constraint_distance()) {
    return;
  }
  // No constraint added if submap pickup is not at desired sampling ratio.
  if (
    !per_submap_sampler_.emplace(

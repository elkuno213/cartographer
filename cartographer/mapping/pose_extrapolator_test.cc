...
TEST(PoseExtrapolatorDeathTest, ExtrapolateInPast) {
  ...
  EXPECT_THAT(
    extrapolator.ExtrapolatePose(time_present),
...
TEST(PoseExtrapolatorTest, EstimateGravityOrientationWithIMU) {
  ...
  sensor::ImuData imu_data{
    current_time, initial_gravity_acceleration, angular_velocity
  };
  ...
  EXPECT_NEAR(
    0.,
    extrapolator->EstimateGravityOrientation(current_time).angularDistance(expected_orientation),
    kPrecision
  );
...
TEST(PoseExtrapolatorTest, ExtrapolateWithPoses) {
  ...
  EXPECT_EQ(
    common::ToUniversal(extrapolator.GetLastPoseTime()),
    common::ToUniversal(current_time)
  );
  EXPECT_THAT(
    extrapolator.ExtrapolatePose(current_time),
...
TEST(PoseExtrapolatorTest, ExtrapolateWithIMU) {
  ...
  for (int i = 0; i < kPoseQueueDuration + 2; ++i) {
    current_time += common::FromSeconds(1);
    extrapolator->AddImuData(
      sensor::ImuData{current_time, initial_gravity_acceleration, initial_angular_velocity}
    );
    extrapolator->AddPose(current_time, current_pose);
  }
  ...
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 10; ++j) {
      current_time += common::FromSeconds(kGravityTimeConstant);
      extrapolator->AddImuData(
        sensor::ImuData{current_time, initial_gravity_acceleration, angular_velocity_yaw}
      );
